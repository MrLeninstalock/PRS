        ...
        if(strncmp(bufferACK,"SYN",3)==0){
            ...
            if(strncmp(bufferACK,"ACK",3)==0){
                ...
                if(fils == 0) {
                    ...
                    //ATTENTION ICI ON RECOIT PAS VRMNT LE FICHIER
                    ...
                    envoi = sendto(descripteurSocketDonnees, bufferEnvoi, sizeof(bufferEnvoi), 0, (struct sockaddr*)&adressePrive, alenPrive);
                    printf("Envoie : %d\nSur le port :%d\n", envoi, htons(adressePrive.sin_port));
                    exit(1);
                } else{
                }
            }
